...
struct AngelScriptModule : IModule {
	AngelScriptModule(Engine& engine, ISystem& system, World& world, IAllocator& allocator)
		: m_engine(engine)
		, m_system(system)
		, m_world(world)
...
struct AngelScriptSystem : ISystem {
	AngelScriptSystem(Engine& engine) : m_engine(engine) {}
	...
	void createModules(World& world) override {
		...
		UniquePtr<AngelScriptModule> module = UniquePtr<AngelScriptModule>::create(allocator, m_engine, *this, world, allocator);
...
{
	return LUMIX_NEW(engine.getAllocator(), AngelScriptSystem)(engine);
}
